...
func _on_FloorMat_start_interaction():
	...
	for i in range(6):
		$FloorMat/Interaction/PushupSprite.texture = load("res://assets/sprites/pushup2.png")
		yield(get_tree().create_timer((i + 1) * 0.5), "timeout")
		if i == 5:
			$FloorMat/Interaction/PushupSprite.texture = load("res://assets/sprites/pushup3.png")
		else:
			$FloorMat/Interaction/PushupSprite.texture = load("res://assets/sprites/pushup1.png")
			$FloorMat/Interaction/PushupCounter.bbcode_text = "%d!" % (i + 1)
